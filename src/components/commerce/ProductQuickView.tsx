...
const ProductQuickView4 = ({
  ...
  return (
    <Dialog>
      <DialogTrigger render={<Button variant="outline" />}>Quick View</DialogTrigger>
      <DialogContent
        style={
          {
    ...
    >
      {options.map((item, index) => (
        <label
          key={index}
          htmlFor={item.id}
          className="relative size-10 shrink-0 cursor-pointer overflow-hidden rounded-md border p-0.5 duration-400 has-checked:ring has-disabled:opacity-60"
        >
          <RadioGroupItem
            id={item.id}
